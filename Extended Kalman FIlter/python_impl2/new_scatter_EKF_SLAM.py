...
class EKF_SLAM:
    def __init__(self):
        ...
        self.map_sigma = 1000*np.eye(2*self.N)
    ...
    def update(self,i):
        ...
        self.df_sliced = self.df_data.iloc[i+self.gaps+1:i+self.gaps+1+self.curr_gap,:]

        self.mu, self.sigma, self.observedLandmarks = correction_step(self.mu, self.sigma, 
                                self.df_sliced, self.observedLandmarks)
        # print("Correction:-")
        # print(self.mu.T)
    ...
    def update_plot(self,i):
        ...
        plt.xlim(-1,11)
        plt.ylim(-1,11)
        plt.title('EKF SLAM timestep = %d' %(i))
        plt.grid()
        ...
        plt.scatter(self.mu[0], self.mu[1], c='r', marker=self.circle, s=150)
        ...
        plt.scatter(landmarks_data[0,:],landmarks_data[1,:],c='k',marker=self.plus,s=150, linewidths=3)
        ...
        for landmark in range(0, self.N):
            if self.observedLandmarks[landmark+1]:
                ...
                plt.plot(self.mu[3+2*landmark,0],self.mu[3+2*landmark+1,0],'bo',markersize=10, linewidth=5)
        ...
        m = self.df_sliced.iloc[0,4]
        for i in range(0,m):
            landmarkiD = int(self.df_sliced.iloc[i,1])
            plt.plot([self.mu[0], self.mu[3+2*(landmarkiD-1)]],[self.mu[1], self.mu[3+2*(landmarkiD-1)+1]],'k')
...
anim = FuncAnimation(fig, func = ekf.update,frames=frames_num,init_func=ekf.init_plot,interval=50,repeat=False)
anim.save("plots/plotting.mp4")

#################
## Used to norm of differences(self.mu(t)-self.mu(t-1))
# print("time.sleep()")
# time.sleep(3)
# plt.cla()
# print(ekf.mu_arr)
